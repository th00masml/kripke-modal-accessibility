...
\begin{enumerate}
  ...
  \item A replication with a leak-free admissible set that confirms the prediction of (a) to the fixture: constrained exact-match falls to 36--37/80, outputs on the unaffected fixtures are unchanged, and the truth-value constant of (c) becomes total for the 0.5B model (Section~\ref{sec:replication}).
  \item A recommended evaluation protocol for constrained decoding on tasks with a semantic ground truth (Section~\ref{sec:discussion}).
\end{enumerate}
...
\paragraph{Canonical output.} \texttt{w\_i |= FORMULA iff TRUE.} or the same with \texttt{FALSE}, with the formula text exactly as given. The pipeline's original admissible set $\mathcal{A}_{\mathrm{gold}}$ is the set of gold judgements of the 80 in-language fixtures. Because several fixtures share a (world, formula) pair, $|\mathcal{A}_{\mathrm{gold}}| = 27$. The consequences of building it this way are the subject of Section~\ref{sec:leak}. The replication in Section~\ref{sec:replication} uses $\mathcal{A}_{\mathrm{prod}}$, the product of the four worlds, four formulas and two truth values, $|\mathcal{A}_{\mathrm{prod}}| = 32$. We refer to the two runs as the \emph{gold run} and the \emph{product run}, and write $\mathcal{A}$ for whichever set a run uses.

\paragraph{Models and arms.} \texttt{Qwen2.5-0.5B-Instruct} and \texttt{Qwen2.5-1.5B-Instruct}, bfloat16, greedy decoding, at most 64 new tokens, via \texttt{transformers} 4.56 on CUDA. Five arms per model: \emph{naive} (model description, task, the fixture text, and the instruction to output one judgement); \emph{prompted} (adds one line giving the exact format); \emph{prompt\_tuned} (adds a five-line template and an explicit instruction to output empty on out-of-language atoms); \emph{constrained} and \emph{constrained\_tuned} (the two prompted arms plus a logits processor that masks every token not consistent with some string in $\mathcal{A}$, resynchronising to the trie root if the prefix leaves the trie, and admitting EOS only once a full canonical string has been emitted, or immediately for out-of-language fixtures). Prompts are in Appendix~\ref{app:prompt}. Every arm ran on all 160 fixtures; 1,600 outputs per run. The gold run predates the analysis below; the product run was executed on 2026-09-17, after it, on the same machine, with \texttt{--allowed-set product} and no other change.
...
\section{Results as the pipeline reports them (gold run)}
\label{sec:primary}

\begin{table}[t]
\centering
\caption{Gold run. Class counts per model and arm as produced by the pipeline's strict scorer (160 fixtures: 80 in-language, 80 out-of-language). Abstention and correct abstention are zero in every cell and omitted. Membership violations count parsed outputs not in $\mathcal{A}_{\mathrm{gold}}$.}
\label{tab:classes}
\footnotesize
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/gold_class_counts.pdf}
\caption{Gold run. Strict migration classes per arm. Constraint arms have no non-canonical outputs and no abstentions; every out-of-language fixture becomes a spurious in-language judgement.}
\label{fig:classes}
\end{figure}

If we stopped here, the conclusion would be that a decode-time constraint roughly doubles the accuracy of small models on modal judgement. Three checks show that this is not what happened, and a re-run confirms the first of them.

\section{Check 1: the constraint set leaks the answer}
\label{sec:leak}

$\mathcal{A}_{\mathrm{gold}}$ contains the 27 distinct gold strings. For a fixture with target world $w$ and formula $\varphi$, the constraint admits \texttt{w |= $\varphi$ iff TRUE.} only if some fixture in the set has that gold judgement, and likewise for FALSE. For 5 of the 16 (world, formula) pairs, only one truth value ever occurs in gold: $(w_2, \Box p)$ is always FALSE, $(w_1, \neg\Box\neg p)$ and $(w_2, \neg\Box\neg p)$ always TRUE, $(w_1, \Box p\wedge\Dia q)$ and $(w_2, \Box p\wedge\Dia q)$ always FALSE. Those pairs cover 25 of the 80 in-language fixtures. On them the trie admits exactly one complete string with the target world and formula, and since both models always reproduce the target world and formula under the constraint (80 of 80, Section~\ref{sec:bias}), the constrained arm cannot be wrong. We call these fixtures \emph{forced} and the other 55 \emph{free}.

\begin{table}[t]
\centering
\caption{Gold run. In-language accuracy split by whether the trie admits both truth values. Constrained arms use strict exact-match; unconstrained arms use the lenient parser of Section~\ref{sec:lenient}. Two-sided binomial $p$ against 0.5 on the free set.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{figs/gold_forced_vs_free.pdf}
\caption{Gold run. Semantic accuracy on forced fixtures (only one truth value admissible) and free fixtures (both admissible). Dotted line: chance. Constrained decoding is perfect where it cannot be wrong and at chance where it can.}
\label{fig:forcedfree}
\end{figure}

The leak is a construction error, not a property of constrained decoding. The admissible set should have been the full product of worlds, formulas and truth values, or, equivalently, the gold strings together with their truth-value complements. If the analysis above is right, replacing $\mathcal{A}_{\mathrm{gold}}$ with that set should cost the constrained arms about 15 exact matches (the 25 forced fixtures falling from certain to chance) and change nothing on the 55 free fixtures, where both truth values were already admissible.

\section{Replication with a leak-free admissible set}
\label{sec:replication}

We added \texttt{--allowed-set product} to the runner and re-ran all five arms on both models with $\mathcal{A}_{\mathrm{prod}}$ (32 strings), same machine, same seeds, greedy decoding. Table~\ref{tab:replication} compares the two runs.

\begin{table}[t]
\centering
\caption{Gold run versus product run, in-language fixtures ($n{=}80$). ``Old forced'' and ``old free'' are the 25 and 55 fixtures of Section~\ref{sec:leak}, which are all free under $\mathcal{A}_{\mathrm{prod}}$. ``Identical'' counts fixtures whose constrained output is byte-identical across the two runs.}
\label{tab:replication}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llrrrrrr}
\toprule
 & & \multicolumn{2}{c}{strict exact} & \multicolumn{2}{c}{old forced (25)} & old free (55) & identical \\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
Model & Arm & gold & product & gold & product & product & old free \\
\midrule
0.5B & constrained & 51 & 36 & 25 & 10 & 26 & 55/55 \\
0.5B & constrained\_tuned & 51 & 36 & 25 & 10 & 26 & 55/55 \\
1.5B & constrained & 52 & 37 & 25 & 10 & 27 & 55/55 \\
1.5B & constrained\_tuned & 52 & 38 & 25 & 12 & 26 & 54/55 \\
\midrule
0.5B & prompted (lenient) & 28 & 29 & 10 & 10 & 19 & 51/55 \\
1.5B & naive (lenient) & 42 & 41 & 12 & 4 & 30 & 44/55 \\
1.5B & prompted (lenient) & 40 & 36 & 13 & 0 & 23 & 43/55 \\
\bottomrule
\end{tabular}
\end{table}

The prediction holds to the fixture. Constrained exact-match falls from 51 to 36 (0.5B) and from 52 to 37 (1.5B). On the 55 fixtures where the leak never applied, the constrained outputs of both models are byte-identical across runs, and the accuracy is the same 26/55 and 27/55. On the 25 formerly forced fixtures, accuracy falls from 25/25 to 10/25 in both models. For the 0.5B model that is exactly what a constant TRUE earns: 10 of the 25 are TRUE in gold, and it says TRUE on all 25. For the 1.5B model the coincidence is misleading. A constant FALSE would earn 15 there; the model says FALSE on 20 and TRUE on 5, and all five departures are wrong. Over all 80 in-language fixtures it departs from FALSE nine times and is right once. Every one of the nine is a $\Box p\wedge\Dia q$ fixture, the formula that is FALSE in 18 of 20 cases. When this model leaves its constant, it does so on the conjunction and gets it wrong 8 times in 9. The tuned variant departs three times, all wrong, hence its 12/25. Overall constrained accuracy under the leak-free set is 36/80 and 37/80, binomial $p$ against 0.5 of 0.43 and 0.58. The pipeline's McNemar test still returns $p=0.004$ and $p=5\times10^{-10}$ in favour of the constraint, because the prompted arm is still scored on strict format; the free-set comparison between lenient prompted and constrained is 0 versus 7 (0.5B) and 3 versus 4 (1.5B) discordant fixtures.

Two things the replication adds that the re-analysis could not. The 0.5B model, freed from the leak, says TRUE on 80 of 80 in-language fixtures under the constraint, which is exactly what it says without the constraint (Section~\ref{sec:bias}); the 15 FALSE answers in the gold run were all forced. And the unconstrained arms are not perfectly reproducible between the two runs: 7 to 36 of 160 outputs per arm differ, with lenient exact-match moving by at most 4 of 80 (1.5B prompted, 40 to 36). Greedy decoding in bfloat16 is deterministic on one machine and one load order, and not across a model reload with a different second model resident; we treat differences of this size as run noise, and note that the constrained outputs on the free set showed none of it.

\section{Check 2: the truth value is a constant}
\label{sec:bias}

The free-set numbers are not merely at chance; they are at chance for a specific reason. Figure~\ref{fig:bias} shows the distribution of the emitted truth value over the 80 in-language fixtures in the product run. The 0.5B model answers TRUE on 80 of 80 under the constraint and on 80 of 80 under the prompted arm. The 1.5B model answers FALSE on 71 of 80 under the constraint, 70 of 80 prompted and 74 of 80 naive. In the gold run the same holds on the free fixtures (55 of 55 TRUE; 51 of 55 FALSE). The constant is a property of the model, not of the arm: the constraint does not change which truth value the model prefers, it only guarantees that the preference is spelled correctly.

\begin{figure}[t]
\centering
\includegraphics[width=0.62\linewidth]{figs/product_truth_value_bias.pdf}
\caption{Product run. Emitted truth value on the 80 in-language fixtures. Gold is 36 TRUE / 44 FALSE. The 0.5B model says TRUE; the 1.5B model says FALSE; the arm barely matters.}
\label{fig:bias}
\end{figure}

This explains the per-formula pattern, which would otherwise look like partial understanding. In the gold run, constrained accuracy on the 0.5B model is 18/20 on $\neg\Box\neg p$ and 8/20 on $\Box p$; on the 1.5B model it is 16/20 on $\Box p\wedge\Dia q$, 17/20 on $\Box p$ and 7/20 on $\Dia q$. Every one of these numbers is the gold base rate of the model's constant: $\neg\Box\neg p$ is TRUE 18 of 20 times and the 0.5B model says TRUE; $\Box p\wedge\Dia q$ is FALSE 18 of 20 times and the 1.5B model says FALSE. A model that had learned that $\Dia$ statements are usually satisfiable in dense frames would show the same profile. So would one that had learned nothing. The fixture design, with its skewed truth distribution per formula, cannot distinguish them, and neither can accuracy.
...
\begin{table}[t]
\centering
\caption{Gold run, in-language fixtures ($n{=}80$): strict versus lenient parsing. ``W+F right'' is the number of outputs whose world and formula match the target under the lenient parser; ``truth right $\mid$ W+F'' is semantic accuracy among those.}
\label{tab:lenient}
\small
\begin{tabular}{llrrrr}
\toprule
...
The 80 out-of-language fixtures test whether a model will decline to judge a formula whose atom has no valuation. The prompt says the atom is outside the language; the tuned prompt says explicitly to output empty. No arm of either model produced an empty output on any of the 80 fixtures, and none on the 80 in-language ones either, in either run (3,200 outputs). Under the constraint, EOS is admissible at step zero for these fixtures, so abstention was available; the models never took it. Instead the constrained arms produce a canonical judgement about $p$ or $q$ on every out-of-language fixture, always with the target world, cycling through the four in-language formulas, since neither admissible set contains anything mentioning $r$. The unconstrained arms at least mention $r$ (54--80 of 80 in the gold run), which a downstream check could catch. The constrained output is fluent, in-language, and about the wrong formula. This is the sharpest form of the problem: the constraint does not just fail to add semantics, it removes the surface evidence that the semantics went wrong.

\section{Discussion}
\label{sec:discussion}

\paragraph{What the constraint bought.} Zero membership violations, zero format violations, and roughly 0.05--0.08 seconds of overhead per item. That is a real engineering benefit if the downstream consumer needs parseable strings. It is not a reasoning benefit, and on this task there was no reasoning to preserve: both models were at chance on the truth value before the constraint and stayed there after it, in both runs.
...
The benchmark is small (160 fixtures, 16 world-formula pairs) and the formulas are shallow (modal depth at most one, except $\neg\Box\neg p$). Truth values are skewed per formula, which is why base-rate matching is hard to separate from understanding; a balanced construction is straightforward and should be preferred. Two models, one family, greedy decoding, one prompt style per arm. The replication is one run on one machine; the run-to-run differences in the unconstrained arms (up to 4 of 80 lenient exact matches) put a floor on how finely any two arms can be compared here. The lenient parser is our own and could be made stricter or looser; the numbers in Table~\ref{tab:lenient} would move a little but the truth-value column would not. Timing numbers are from a single GPU run and are indicative only.

\section{Conclusion}

A byte-trie constraint on two small Qwen2.5 models produced perfectly canonical Kripke judgements and, by the pipeline's own scorer, doubled exact-match accuracy with $p<10^{-7}$. On inspection, 25 of the 80 in-language fixtures admitted only one truth value under the constraint; on the other 55 the constrained arms were at chance; each model emitted a near-constant truth value in every arm; and lenient parsing showed the unconstrained models already had the structure right. A re-run with a leak-free admissible set dropped constrained accuracy to 36/80 and 37/80 and left the 55 unaffected fixtures byte-identical. The constraint changed the spelling of a guess. Evaluation of constrained decoding should split fixtures by whether the constraint leaves the answer open, compare emitted marginals against gold marginals, and parse unconstrained outputs leniently before crediting the decoder with anything semantic.

\section*{Reproducibility}
\label{sec:repro}

Fixtures, the trie constraint, the \texttt{transformers} binding, the runner, the scorer, the cached outputs of both runs (\texttt{outputs/gold/}, \texttt{outputs/product/}) and the re-analysis script (\texttt{src/analysis\_paper.py --run gold|product}, which writes \texttt{paper\_stats.json} and every figure here) are at \url{https://github.com/th00masml/kripke-modal-accessibility} \citep{nawara2026kripke}. Fixtures are synthetic and released under CC0. \texttt{run.py --allowed-set product} builds the leak-free admissible set; the default \texttt{gold} reproduces the original construction. Re-running the analysis needs NumPy and Matplotlib and takes seconds.
